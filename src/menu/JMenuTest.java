...
public class JMenuTest extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (menu == mnNew) {
			...
			(this, "변경 내용을 제목없음에 저장하시겠습니까?", "메모장", JOptionPane.DEFAULT_OPTION, -1, null, options, options[0]);
...
//				try (BufferedReader reader = new BufferedReader(new FileReader(file.getPath()))){}
				try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file.getPath()), "ms949"))){
					String msg;
...
//			textArea.replaceSelection(format.format(new Date()));
	...
	private void save() {
		...
		int retVal = chooser.showSaveDialog(this);	// int : 저장 & 취소 버튼이 있기 때문에 타입을 맞춰 준다
		if (retVal == 0) {	// 저장
			...
			// 현재 textArea 값을 파일로 저장한다. BufferdWriter(FileWriter) 사용
			try (BufferedWriter bw = new BufferedWriter(new FileWriter(file.getPath()))){
				String str = textArea.getText();
				System.out.println(file.getPath());	// C:\Users\admin\Documents\readme1.txt
